...
public class UserController extends BaseController {
	...
	@RequestMapping(method=RequestMethod.GET, value="leftmenus")
	public @ResponseBody String getLeftMenu(@RequestParam int menuId, HttpSession session) throws Exception {
		Gson gson = new Gson();
		ArrayList<Menu> menus = menuService.getImmediateChildren(menuId);
		return gson.toJson(menus);
	}
	
	private List<Menu> getMenui18N(ArrayList<Menu> menus) {
		for (Menu menu : menus) {
			menu.setMenuLabel(getText(menu.getMenuLabel()));		// Make sure the menu label in db should not have space as it's going to be a key
		}
		return menus;
	}
